...
**Q：我好像没点「同步更改」，GitHub 上却已经变了 —— 是它自动传的吗？**
A：**不是自动的，一定是某次 `push` 真的执行过。**

VS Code 默认只做一件事：**每 3 分钟自动"拉取检查"一次**（autofetch）。这个动作是**只读**的，
只更新本地关于远端的记录，**永远不会往 GitHub 上传**。所以：

> **GitHub 上出现变化 ⇒ 一定有人执行过 `git push`。**
> 它不可能自己飞上去，`commit` 也不会碰 GitHub。

最常见的真相是**你其实点过，只是没察觉**：凭据已经登录过（不会弹窗），点下去一两秒就完成，
既没有成功提示，也没有进度条，唯一的反馈是状态栏那个 `↑n` 徽标悄悄消失。

**想知道"到底什么时候被推上去的"**，用这条命令查远端引用日志：
...
| 日志里写什么 | 含义 |
|---|---|
| `update by push` | 发生过一次**推送** —— 这就是 GitHub 变化的原因 |
| `fetch` | 只是**拉取检查**，不影响 GitHub |

真实输出示例：

```
5bd257b refs/remotes/origin/main@{2026-09-16 02:11:21 +0800}: update by push
cbf3a92 refs/remotes/origin/main@{2026-09-16 02:00:27 +0800}: update by push
```
